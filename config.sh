...
# ════════════════════════════════════════════════════════════
# موضع اللوجو من حافة الشاشة (بالنسبة المئوية %)
# ════════════════════════════════════════════════════════════
# 
# LOGO_MARGIN_X = المسافة الأفقية من الحافة (% من عرض الشاشة)
# LOGO_MARGIN_Y = المسافة العمودية من الحافة (% من ارتفاع الشاشة)
#
# 📐 أمثلة توضيحية بسيطة:
#
# ┌─────────────────────────────────────┐
# │  LOGO_MARGIN_X="1"  (1%)            │  لوجو ملاصق للحافة تقريباً
# │  LOGO_MARGIN_Y="1"                  │  (~19 بكسل في 1080p)
# └─────────────────────────────────────┘
#
# ┌─────────────────────────────────────┐
# │  LOGO_MARGIN_X="2"  (2%)            │  مسافة صغيرة
# │  LOGO_MARGIN_Y="3"                  │  (~38 بكسل أفقي، ~32 عمودي)
# └─────────────────────────────────────┘
#
# ┌─────────────────────────────────────┐
# │  LOGO_MARGIN_X="5"  (5%)            │  مسافة متوسطة
# │  LOGO_MARGIN_Y="5"                  │  (~96 بكسل أفقي، ~54 عمودي)
# └─────────────────────────────────────┘
#
# 💡 نصائح سريعة:
#    1-2%  = ملاصق للحافة
#    3-5%  = مسافة صغيرة ومريحة ⭐ موصى به
#    6-10% = مسافة واضحة
#
LOGO_MARGIN_X="1"    # النسبة الأفقية من عرض الشاشة (1% = ~19px في 1080p)
LOGO_MARGIN_Y="5"    # النسبة العمودية من الارتفاع (5% = ~54px في 1080p)
...
build_logo_filter() {
    ...
    # تحويل النسب المئوية إلى بكسل بناءً على دقة الشاشة
    # افتراضياً 1920x1080، ولكن يمكن تغييره حسب RESOLUTION
    local margin_x_px=$((1920 * LOGO_MARGIN_X / 100))
    local margin_y_px=$((1080 * LOGO_MARGIN_Y / 100))
    
    local position_filter=""
    case $LOGO_POSITION in
        topleft)
            position_filter="x=$margin_x_px:y=$margin_y_px"
            ;;
        topright)
            position_filter="x=W-w-$margin_x_px:y=$margin_y_px"
            ;;
        bottomleft)
            position_filter="x=$margin_x_px:y=H-h-$margin_y_px"
            ;;
        bottomright)
            position_filter="x=W-w-$margin_x_px:y=H-h-$margin_y_px"
            ;;
        *)
            position_filter="x=W-w-$margin_x_px:y=$margin_y_px"
            ;;
